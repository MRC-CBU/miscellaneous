...
This is a primer on simple linear statistical models for relating Age, Brain and Cognition (or indeed any three variables A, B and C). The outcome variable we care about is C, whereas the A variable is generally assumed to be the main cause of variation in C, and one question is how this covariation relates to B. The focus is on different ways of modelling A, B and C, particularly how to treat A (age). We start with cross-sectional data on age, and then turn to longitudinal measures. 

** Note: Longitudinal Section not started yet!! **

## Cross-sectional measures of Age

Let's start by assuming with have one value for A, B and C from each of "npt" participants (later we will consider latent factors for A, B and C derived from multiple measures of each, as well as repeated measures from each participant). We'll start with defining the Age variable (A) as being drawn from a Gaussian distribution: 
...
#### 1.2. Independent Causes

We can also create second dataset (``df2``) where A independently causes B and C (model M2 in Figure 1):
...
...this is not the case when A is added as a covariate:
...
The Chi-square test shows that the model cannot be rejected (i.e, fits reasonably well). Compare this to the fit of a second (incorrect) model in which A independently affects B and C, as below: 
...
#### 2.2. Independent Causes

We can also do the converse, ie compare SEM1 and SEM2 in their ability to fit data generated by model M2 with independent causes (where SEM2 is now the correct model). 
...
This time, the Chi-squareed, AIC and BIC all favour SEM2 instead, as expected (indeed, if you uncomment the preceding "anova" call, you will see that SEM1 is correctly rejected by the data).
...
anova(SEM3_df1)
```
we can see that SEM3 is correctly rejected by the data, but SEM1 is not. This is because SEM1 predicts greater covariance between B and C than between A and C (as found in data, given their sequential generation), whereas SEM3 predicts opposite of greater covariance between A and C than between B and C. In other words, SEM can provide some insight on the possible causal direction of relationships between variables (the "watershed" approach, e.g., [Kievit et al. 2016](https://doi.org/10.1016/j.neuropsychologia.2016.08.008)). 

However, you will note that the AIC and BIC are actually lower for SEM3. This relates to the scaling of the data, which affects AIC/BIC (but not Chi-squared): The B variable has greater spread than the A variable (since B is generated by adding independent variance to A), so the residuals of the model fit will be numerically smaller when B is the exogeneous variable (at base of SEM3), leading to better AIC/BIC, even though the proportion of total variance explained is less. This can be addressed by re-scaling (Z-scoring) all the variables first, as illustrated with the commented code below. 
...
The Chi-square difference is too small to be significant (given the difference in 1 df), and the BIC is lower for SEM1 than SEM4, suggesting that the extra parameter is not needed, i.e, the more parsimonious SEM1 model is more likely to be the true model.
...
which confirms that the ``AB`` path on ``C`` is significant, as expected. 
